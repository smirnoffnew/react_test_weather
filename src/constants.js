// export const API_KEY = 'Rx4P2bUn4OzXTKvGBVHggsiMw5sKfRsm';  
// export const API_KEY = 'j7dEtdKKOnRiMrXnItTkwUdh6Jg2dX4F'; 
// export const API_KEY = 'DgtqJlrmTGsN0TyUumgEAvJSn9Je2uhl';

export const API_KEY = 'ASWcvsmMjzKodukBPmM4NMJY6mFIRoA5';


export const BASE_URL = 'http://dataservice.accuweather.com'


export const ADD_LOCATION = 'ADD_LOCATION';
export const REMOVE_LOCATION = 'REMOVE_LOCATION';

export const TOGGLE_TEMPERATURE_TYPE = 'TOGGLE_TEMPERATURE_TYPE';


